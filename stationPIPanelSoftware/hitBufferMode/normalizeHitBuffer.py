...
def main():
    ...
    for i in range(subruns):
        ...
        #dump = cmdLoop('dump_hits_binary', ser, ntry=10, decode=False)
        #if dump is not None:
        ...
        rate = getRate(ser)
        print(f'Panel {activePanel} trigger rate = {rate} Hz')
        if rate > 0.0:
...
def getNextRun(port, runsdir='runs'):
    ...
    if os.path.exists(os.path.join(here, runsdir)):
        lastrun = sorted(glob.glob(os.path.join(here, runsdir, 'port_'+port+'_'+'run_*')))
        if lastrun:
        ...
        lastrun = 0
    run = 'port_{0}_run_{1}'.format(port,str(lastrun+1).zfill(7))
