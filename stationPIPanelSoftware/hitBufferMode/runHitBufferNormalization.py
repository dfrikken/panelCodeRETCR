...
def main():
    ...
    #find out which panels we're talking to
    ...
    port0FileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port0Panel}ThresholdSweep.txt'
    port1FileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port1Panel}ThresholdSweep.txt'

    port0BiasFileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port0Panel}SweepBias.txt'
    port1BiasFileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port1Panel}SweepBias.txt'
    ...
    if not (os.path.isfile(port0FileName) and os.path.isfile(port1FileName)):
        ...
        print(f'Full station trigger rate normalization complete. ignore error below please :)')
    ...
    if (getThresholds(desiredRate,port0SweepList,port1SweepList)) :
        port0BiasFileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port0Panel}SweepBias.txt'
        port1BiasFileName = f'/home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/panel{port1Panel}SweepBias.txt'
        biasValues = getBiasVoltage(port0BiasFileName, port1BiasFileName)
        print(f'bias voltages at {biasValues}')
...
def getBiasVoltage(port0BiasFileName, port1BiasFileName):
    ...
    port1SweepList = []
    port1File = open(port1BiasFileName,'r')
...
def thresholdSweep(port0Panel,port1Panel):
    ...
    for i in range(101):
        ...
        runHistString = f'python /home/retcr/deployment/stationPIPanelSoftware/hitBufferMode/normalizeHitBuffer.py -p 0 -d {threshold} -v {biasValues[0]} -t {runTime} -s {runTime} & python normalizeHitBuffer.py -p 1 -d {threshold} -v {biasValues[1]} -t {runTime} -s {runTime} &'
